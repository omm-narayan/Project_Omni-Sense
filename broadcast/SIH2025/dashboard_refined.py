...
PRESENCE_THRESHOLD = 65   # Changed to 65% minimum
SMOOTH_WINDOW = 5         # number of readings for smoothing

# ====== Serial Reading Thread ======
def read_serial():
    global co2_values, presence_values
    buffer = []
    while True:
        try:
            ...
            if ',' in line:
                ...
                # threshold after smoothing - use 65% threshold
...
def generate_presence_graph():
    ...
    while True:
        ...
        # Changed Y-axis to 65-100%
        ax.set_ylim(65, 100)
        ax.set_title("Presence Sensor")
        ax.set_ylabel("%")
        ax.set_xlabel("Samples")
        buf = io.BytesIO()
        fig.savefig(buf, format='png')
